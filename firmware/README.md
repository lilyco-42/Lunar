...
基于 ESP32-C3 的多传感器采集终端，6 个环境传感器实时读数显示在 0.96" OLED 屏幕上。

## 硬件

| 传感器 | 型号 | 接口 | 引脚 | 用途 |
|--------|------|------|------|------|
| 显示屏 | SSD1306 0.96" OLED | I2C (0x3C) | SCL=GPIO4, SDA=GPIO5 | 仪表盘显示 |
| 光照 | BH1750 | I2C (0x23) | 同上 | 环境光强度 |
| 电源监测 | INA219 | I2C (0x40) | 同上 | 电压/电流/功率 |
| 麦克风 | MAX9814 | ADC | GPIO0 (ADC1_CH0) | 声音强度 |
| 温湿度 | DHT11 | OneWire-style | GPIO7 | 空气温湿度 |
| 温度探头 | DS18B20 | OneWire | GPIO6 | 水温/物体温度 |
| 一氧化碳 | MQ-7 | ADC | GPIO1 | 待接入 |
| 空气质量 | MQ-135 | ADC | GPIO3 | 待接入 |
| 音频输出 | PCM5102 + PAM8403 | I2S | GPIO18-20 | 待接入 |
...
DS18B20 DATA ───┤ GPIO6             │──── (需 4.7kΩ 上拉)
 DHT11 DATA ────┤ GPIO7             │
        [空闲] ─┤ GPIO8             │
        [空闲] ─┤ GPIO9             │
        [空闲] ─┤ GPIO10            │──── (被 flash 占用，不可用)
                │                   │
I2S BCLK ───────┤ GPIO18            │──── (待接 PCM5102)
I2S LRCLK ──────┤ GPIO19            │──── (待接 PCM5102)
I2S DOUT ───────┤ GPIO20            │──── (待接 PCM5102)
                │                   │
                │ 3.3V ─── OLED/BH1750/INA219/MAX9814/DS18B20/DHT11
                │ 5V ───── MQ-7/MQ-135/PAM8403
                │ GND ──── 所有设备共地
                └───────────────────┘
```

> **注意**：DS18B20 DATA 线需外接 4.7kΩ~10kΩ 上拉电阻到 3.3V。
> MQ 系列传感器需 5V 供电，AO 输出 0~5V 需经分压到 0~2.5V 再入 ADC。
